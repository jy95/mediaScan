...
// local import
import {
  filterMoviesByProperties,
  filterTvSeriesByProperties
} from "./filters/filterProperties";
import * as MediaScanTypes from "./MediaScanTypes";
import {
  defaultParser,
  defaultWhichCategoryFunction,
  promisifiedAccess
...
class MediaScan extends EventEmitter {
  ...
  public static readonly TV_SERIES_TYPE = MediaScanTypes.Category
    .TV_SERIES_TYPE;
  // properties
  ...
  protected stores: MediaScanTypes.MapSet<
    MediaScanTypes.TPN | MediaScanTypes.TPN_Extended
  >;
      ...
      try {
        ...
        if (scanningResult[MediaScan.MOVIES_TYPE] !== undefined) {
          this.stores.set(
            ...
            new Set([
              ...this.allMovies,
              ...scanningResult[MediaScan.MOVIES_TYPE]
            ])
          );
        }
        ...
        if (scanningResult[MediaScan.TV_SERIES_TYPE] !== undefined) {
          ...
          const newSeries = reduce(
            scanningResult[MediaScan.TV_SERIES_TYPE],
            (result, tvSeries) => {
          ...
          forIn(newSeries, (seriesArray, seriesName) => {
            const resultSet = newTvSeries.has(seriesName)
              ? newTvSeries.get(seriesName)
              : new Set();
            newTvSeries.set(
              seriesName,
  ...
  public addNewPath(...paths: string[]): Promise<any> {
    ...
    return new PromiseLib((resolve, reject) => {
      PromiseLib.map(paths, path => promisifiedAccess(path))
        .then(() => {
  ...
  public scan(): Promise<any> {
    return new PromiseLib((resolve, reject) => {
      FileHound.create()
        ...
        .find()
        .then(files =>
          PromiseLib.join(this.addNewFiles(files), () => {
  ...
  public removeOldFiles(...files: string[]): Promise<any> {
    return new PromiseLib((resolve, reject) => {
      try {
        ...
        const filterContentType = requestedType => file =>
          file.category === requestedType;
        ...
        // movies files
        const moviesFiles = filter(
          mappedFiles,
          filterContentType(MediaScan.MOVIES_TYPE)
        );
        ...
        if (moviesFiles.length > 0) {
          ...
          this.stores.set(
            ...
            new Set(
              filter(
                ...this.allMovies,
                movie => !some(moviesFilePaths, movie.filePath)
              )
            )
          );
        }
        ...
        // tv-series
        const seriesFiles = filter(
          mappedFiles,
          filterContentType(MediaScan.TV_SERIES_TYPE)
        );
        ...
        if (seriesFiles.length > 0) {
          ...
          const seriesShows = compose(
            ...
            pluck(series => {
              return {
                ...series,
                seriesName: this.parser(series.filePath).title
              };
            })
          )(seriesFiles);
          ...
          forIn(seriesShows, (seriesArray, seriesName) => {
            const previousSet = newTvSeries.has(seriesName)
              ? newTvSeries.get(seriesName)
              : new Set();
            const filteredSet: Set<MediaScanTypes.TPN_Extended> = new Set(
              filter(
                [...previousSet],
                episode => !includes(seriesArray, episode.filePath)
              )
            );
            ...
            if (filteredSet.size === 0) {
              ...
              newTvSeries.set(seriesName, filteredSet);
            }
          });
  ...
  get allMovies(): Set<MediaScanTypes.TPN_Extended> {
    return this.stores.get(MediaScan.MOVIES_TYPE) as Set<
      MediaScanTypes.TPN_Extended
    >;
  }

  get allTvSeries(): Map<string, Set<MediaScanTypes.TPN_Extended>> {
    return this.stores.get(MediaScan.TV_SERIES_TYPE) as Map<
      string,
      Set<MediaScanTypes.TPN_Extended>
    >;
  }
  ...
  public toJSONObject(looseMode?: boolean): MediaScanTypes.LibAsJson {
    ...
    const toBeSerialized = looseMode
      ...
      : [
          ...
          [
            "series",
            this.allTvSeriesNames.reduce((acc, currentSeries) => {
              acc.push([
                currentSeries,
                [...this.allTvSeries.get(currentSeries)]
              ]);
              return acc;
            }, [])
          ]
  ...
  public static createFromJSON(
    json: MediaScanTypes.LibAsJson,
    customConfig?: MediaScanTypes.CustomFunctionsConfig
  ): MediaScan {
